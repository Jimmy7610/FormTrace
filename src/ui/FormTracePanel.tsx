...
import { buildMarkdownReport } from '../analyzer/buildMarkdownReport';
import { buildGitHubIssueReport } from '../analyzer/buildGitHubIssueReport';
import { buildJiraReport } from '../analyzer/buildJiraReport';
import { analyzeSession } from '../analyzer/analyzeSession';
import { normalizeReportForHiddenRequiredFields } from '../analyzer/normalizeReport';
import { filterTechnicalDetailsForDebugMarkers } from '../analyzer/filterDebugMarkers';
import {
  getReportHistory,
  saveReportHistory,
  deleteSavedReport,
...
export default function FormTracePanel({ isPersistent = false }: { isPersistent?: boolean }) {
  ...
  useEffect(() => {
    chrome.storage.local.get(['debugMarkersVisible', 'formtracePersistentWindowMode'], (result) => {
  ...
  const handleTogglePersistentMode = (e: React.ChangeEvent<HTMLInputElement>) => {
    const nextVal = e.target.checked;
      ...
      {!isPersistent && (
        <div className="settings-area">
          <div className="settings-label-container">
            ...
            <span className="settings-helper">Opens FormTrace in a separate window that stays open until you close it.</span>
          </div>
          <input
            type="checkbox"
            id="chk-persistent-mode"
            className="settings-checkbox"
            checked={persistentWindowMode}
            onChange={handleTogglePersistentMode}
          />
        </div>
      )}
